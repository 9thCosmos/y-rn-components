...
//  */
// const getBitValue: (num: number, idx: number) => number = (num, idx) => (num & (1 << idx)) >> idx;
...
//  */
// const changeBitValue: (num: number, idx: number) => number = (num, idx) => num ^ (1 << idx);
...
//  */
// const setBitValueWithOne: (num: number, idx: number) => number = (num, idx) => {
...
//  */
// const setBitValueWithZero: (num: number, idx: number) => number = (num, idx) => {
//     const digit = getBitValue(num, idx);
//     return num + -1 * digit * (1 << idx);
// };
...
//  */
// const bytesToHexString: (bytes: number[]) => string = (bytes) => {
//     return bytes
//         .map((x) => {
...
//  */
// const highLowToInt: (high: number, low: number) => number = (high, low) => low + (high << 8);
...
//  */
// const intToHighLow: (num: number) => [number, number] = (num) => [num >> 8, num & 0xff];
...
//  */
// const inMaxMin: (min: number, max: number, value: number) => number = (min, max, value) =>
...
//  * @returns {Number}
//  */
// const scaleNumber: (scale: number, value: number) => string = (scale, value) =>
//     (value / Math.pow(10, scale)).toFixed(scale).toString();
...
//  */
// const range: (start: number, end: number, step?: number) => number[] = (start = 0, end, step = 1) => {
//     let index = -1;
...
//  */
// const calcPosition: (value: number, min: number, max: number, newMin: number, newMax: number) => number = (
//     value,
//     min,
//     max,
//     newMin,
...
//  */
// const calcPercent: (min: number, max: number, value: number, offset?: number) => number = (
//     min,
//     max,
//     value,
//     offset = 0
...
const calNumLength: (number1: number, number2: number) => number = (number1, number2) => {
...
const mul: (number1: number, number2: number) => number = (number1, number2) => {
    const maxLength = calNumLength(number1, number2);
    return (number1 * maxLength * number2 * maxLength) / maxLength / maxLength;
};
...
const div: (number1: number, number2: number) => number = (number1, number2) => {
    const maxLength = calNumLength(number1, number2);
    const n1 = number1 * maxLength;
    const n2 = number2 * maxLength;
    return n1 / n2;
};
